...
require("fzf-lua").setup({
    ...
    files = {
      prompt = 'Files> ',
      cmd = 'rg .  --files --no-ignore --hidden --follow -g "bin/*" -g "*.{ts,tsx,graphql,coffee,haml,hamlc,erb,js,json,rs,go,rb,py,swift,scss,c,yml,yaml,css}" -g "!{.git,generated,node_modules,dist,vendor,log,swp,tmp,venv,__pychache__,pyc}/*"',
    ...
    grep = {
      ...
      rg_opts = "--hidden --column --line-number --no-heading --color=always --smart-case -g 'bin/*' -g '*.{ts,tsx,graphql,coffee,haml,hamlc,erb,js,rs,go,rb,py,swift,scss,c}' -g '!{.git,**/generated,node_modules,dist,vendor,log,swp,tmp,venv,__pychache__,pyc}/*'",
      actions = file_actions,
    },
    bcommits = {
      previewer = 'bat'
